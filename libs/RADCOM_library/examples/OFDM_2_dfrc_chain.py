...
preamble_R = choice(const,(N_sc,))
symbols_C = choice(const,(P,N_sc))
preamble_C = choice(const,(N_sc,))
...
targets_params = [[1,0,2.75*M/fs_rx]]# generate_targets(N_max,f_D_max,tau_max,random_amplitude=False) 
...
path_params = [[1*exp(1j*1*pi/4), 1000, 2e-08]] # (0.0003605130522449762+0.00023168767043800912j)
tau_sync = path_params[0][2]

# OFDM DFRC signal generation
tx_sig_R = SISO_OFDM_DFRC_TX(symbols_R, preamble_R, L_CP, L, periodic_preamble=False)
tx_sig_C = SISO_OFDM_DFRC_TX(symbols_C, preamble_C, L_CP, L, periodic_preamble=False)
...
plt.figure()
plt.subplot(211)
plt.plot(np.real(tx_sig_R))
plt.subplot(212)
plt.plot(np.real(rx_sig_R))
...
## COMM PROCESSING
# OFDM COMM receiver
symbols_hat,path_params_hat = SISO_OFDM_DFRC_COMM_RX(rx_sig_C, preamble_C, symbols_C[pilots_idx_t_mesh.T,pilots_idx_f_mesh.T], N_sc, L_CP, P, Nt, Nf, fs_rx, M, perfect_pulse_sync, tau_sync, perfect_CSI, path_params,zeropad_doppler=1,zeropad_delay=1,ofdm_synchronisation=False,doppler_pilots=doppler_pilots)
